...
def _collect_image_paths(config: FCMAEDataConfig, *, manifest_path: str | None = None) -> list[Path]:
    extensions = {extension.lower() for extension in config.extensions}
    if manifest_path is not None:
        return _collect_manifest_paths(manifest_path)
    ...
    return _collect_manifest_paths(config.manifest_path)


def _collect_manifest_paths(manifest_path: str) -> list[Path]:
...
class FCMAEImageDataset(Dataset[dict[str, Any]]):
    def __init__(
        ...
        *,
        ...
        self.paths = _collect_image_paths(config, manifest_path=manifest_path)
        self._warned_unreadable_paths: set[Path] = set()
        if not self.paths:
            source = manifest_path or config.image_dir or config.manifest_path
...
class FCMAEDataModule(L.LightningDataModule):
    def __init__(
        ...
        *,
        ...
        self.val_set: FCMAEImageDataset | None = None
        self.preview_set: FCMAEImageDataset | None = None

    @property
    def has_validation(self) -> bool:
        return self.data_config.validation_manifest_path is not None
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if self.val_set is None and self.data_config.validation_manifest_path is not None:
            self.val_set = FCMAEImageDataset(
                self.data_config,
                patch_size=self.patch_size,
                encoder_stride=self.encoder_stride,
                manifest_path=self.data_config.validation_manifest_path,
            )
        if self.preview_set is None and self.data_config.preview_manifest_path is not None:
            self.preview_set = FCMAEImageDataset(
                self.data_config,
                patch_size=self.patch_size,
                encoder_stride=self.encoder_stride,
                manifest_path=self.data_config.preview_manifest_path,
            )
