...
class FCMAEReconstructionLogger(L.Callback):
    ...
    def __init__(
        ...
        *,
        ...
        max_images: int,
        payload_attribute: str = "_latest_preview",
        log_key: str = "train/reconstruction_preview",
    ) -> None:
        super().__init__()
        if every_n_steps < 1:
    ...
    def on_train_batch_end(
        ...
        payload = getattr(pl_module, self.payload_attribute, None)
        if not payload:
            return
        setattr(pl_module, self.payload_attribute, None)
    ...
    def on_validation_batch_end(
        ...
        if self.payload_attribute != "_latest_val_preview":
            return
