...
class BenchmarkRunnerConfig:
    ...
    ours_checkpoint: str = "weights/GrandStaff/transcoda-model.ckpt"
...
class BenchmarkRunner:
    ...
    def _validate_metric_dependencies(self) -> None:
        """Fail before inference if a selected metric cannot run in this environment."""
        if "omr_ned" in self.selected_metrics and not is_musicdiff_available():
            raise RuntimeError(
                "OMR-NED metric was requested, but `musicdiff` is not installed or cannot be "
                "imported. Run `uv sync --group omr-ned` before launching the benchmark."
            )

    def run(self) -> dict:
        """Execute the full benchmark run."""
        self._validate_metric_dependencies()
