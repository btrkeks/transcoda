...
def project_kern_to_pitches(text: str) -> str:
    """Project **kern text to pitch-only content.

    Non-data records such as interpretations, barlines, and comments are
    dropped. Retained data lines preserve their tab-separated field structure
    and space-separated chord structure.
    """
    if not text:
        return text
    ...
    for line in text.splitlines():
        ...
        projected_fields = [_project_kern_field_to_pitches(field) for field in line.split("\t")]
        if any(field for field in projected_fields):
            output_lines.append("\t".join(projected_fields))
...
def _project_kern_field_to_pitches(field: str) -> str:
    projected_tokens: list[str] = []
    for token in field.split(" "):
        projected = _project_kern_token_to_pitch(token)
        if projected is not None:
            projected_tokens.append(projected)
    return " ".join(projected_tokens)


def _project_kern_token_to_pitch(token: str) -> str | None:
    if token == ".":
        return "."
    if _is_rest_token(token):
        return "r"
    return extract_pitch(token)


def _is_rest_token(token: str) -> bool:
    stripped = token.lstrip("[](){}_.0123456789")
    return stripped.startswith("r")
