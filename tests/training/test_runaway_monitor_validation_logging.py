...
from src.training.lightning_module import TranscodaTrainer, _RepeatLoopFilteredSampleCounts
...
class _ValidationTrainerStub:
    ...
    def should_log_validation_examples(self):
        return TranscodaTrainer.should_log_validation_examples(self)

    def _validation_set_metric_name(self, set_name, metric_name):
        return TranscodaTrainer._validation_set_metric_name(self, set_name, metric_name)

    def _validation_aggregate_metric_name(self, metric_name):
        return TranscodaTrainer._validation_aggregate_metric_name(self, metric_name)
...
def test_validation_step_updates_runaway_tracker():
    ...
    outputs = TranscodaTrainer.validation_step(stub, batch, batch_idx=0, dataloader_idx=0)
...
def test_validation_step_updates_polish_cer_without_repeat_loops():
    ...
    TranscodaTrainer.validation_step(stub, batch, batch_idx=0, dataloader_idx=0)
...
def test_on_validation_epoch_end_logs_per_set_and_overall_runaway_metrics():
    ...
    TranscodaTrainer.on_validation_epoch_end(stub)
