...
from src.training.lightning_module import TranscodaTrainer, _finite_estimated_stepping_batches
...
def test_on_fit_start_compiles_when_enabled(monkeypatch):
    ...
    TranscodaTrainer.on_fit_start(stub)
...
def test_configure_gradient_clipping_rejects_unknown_algorithm():
    ...
    with pytest.raises(ValueError, match="Unsupported gradient_clip_algorithm"):
        TranscodaTrainer.configure_gradient_clipping(
            module,
...
def test_disable_compiled_forward_model_allows_strict_restore_of_stripped_checkpoint():
    ...
    disabled = TranscodaTrainer.disable_compiled_forward_model(stub)
...
def test_training_step_uses_compiled_forward_path():
    ...
    loss = TranscodaTrainer.training_step(stub, batch, _batch_idx=0)
...
def test_generate_path_stays_on_eager_model(monkeypatch):
    ...
    preds = TranscodaTrainer._generate_with_grammar(
        stub,
...
def test_generate_path_expands_encoder_state_for_beam(monkeypatch):
    ...
    stub = _TrainerGenerateStub()
    pixel_values = torch.randn(2, 3, 8, 8)
    _ = TranscodaTrainer._generate_with_grammar(
        stub,
