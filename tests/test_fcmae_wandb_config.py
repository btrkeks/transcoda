...
def test_fcmae_config_accepts_train_and_validation_manifests() -> None:
    payload = _base_config()
    payload["data"] = {
        "image_dir": None,
        "manifest_path": "data/fcmae/manifests/train.txt",
        "validation_manifest_path": "data/fcmae/manifests/validation.txt",
        "preview_manifest_path": "data/fcmae/manifests/preview_fixed.txt",
    }
    config = FCMAEConfig.model_validate(payload)
    assert config.data.manifest_path == "data/fcmae/manifests/train.txt"
    assert config.data.validation_manifest_path == "data/fcmae/manifests/validation.txt"
    assert config.data.preview_manifest_path == "data/fcmae/manifests/preview_fixed.txt"
    assert config.training.validation_every_n_steps == 1000
    assert config.training.validation_seed == 17
