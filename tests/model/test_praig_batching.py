...
PRAIG_PATH = Path("external/models/praig-smt").resolve()
if str(PRAIG_PATH) not in sys.path:
    sys.path.insert(0, str(PRAIG_PATH))

smt_model_module = pytest.importorskip(
    "smt_model",
    reason="PRAIG SMT external checkout is not available",
)
praig_trainer_module = pytest.importorskip(
    "smt_trainer",
    reason="PRAIG SMT external checkout is not available",
)
SMTConfig = smt_model_module.SMTConfig
SMTModelForCausalLM = smt_model_module.SMTModelForCausalLM
