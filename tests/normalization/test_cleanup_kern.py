...
class TestRemoveTenutoMarks:
    """Tests for tenuto mark removal in CleanupKern."""

    def test_removes_tenuto_from_note_fields(self):
        """Should strip tenuto marks from note data fields."""
        pass_obj = CleanupKern()
        ctx = NormalizationContext()
    ...
    def test_removes_tenuto_from_chord_subtokens(self):
        """Should strip tenuto marks from individual notes inside chords."""
        pass_obj = CleanupKern()
        ctx = NormalizationContext()
    ...
    def test_removes_tenuto_while_preserving_trailing_markers(self):
        """Should strip only tenuto while preserving beams, slurs, and ties."""
        pass_obj = CleanupKern()
        ctx = NormalizationContext()
    ...
    def test_preserves_interpretation_and_barline_fields_with_tilde(self):
        """Should not alter interpretation or barline fields containing tildes."""
        pass_obj = CleanupKern()
        ctx = NormalizationContext()

        input_text = "*custom~\t*clefG2\n=~\t=\n4c~\t4e~"
        expected = "*custom~\t*clefG2\n=~\t=\n4c\t4e"
