...
def test_run_inference_profile_summary_compares_to_opposite_constraint_run(tmp_path: Path) -> None:
    ...
    counterpart_summary = {
        "metadata": {
            ...
            "checkpoint_path": "weights/GrandStaff/transcoda-model.ckpt",
...
def test_runner_passes_layout_normalization_to_ours_adapter(monkeypatch, tmp_path: Path) -> None:
    ...
    assert captured["checkpoint_path"] == "weights/GrandStaff/transcoda-model.ckpt"
...
def test_legato_model_init_loads_nested_vision_model(monkeypatch) -> None:
    legato_path = Path("models/external/legato").resolve()
    if not legato_path.exists():
        pytest.skip("LEGATO external checkout is not available")

    monkeypatch.syspath_prepend(str(legato_path))
    modeling_legato = pytest.importorskip(
        "legato.models.modeling_legato",
        reason="LEGATO external checkout is not importable",
    )
...
def test_legato_adapter_overrides_encoder_path(monkeypatch, tmp_path: Path) -> None:
    ...
    class FakeLegatoProcessor:
        def __init__(self, *, image_processor, tokenizer):
            self.image_processor = image_processor
            self.tokenizer = tokenizer
            self.model_id = tokenizer.model_id

    fake_legato_package = ModuleType("legato")
    fake_legato_models = ModuleType("legato.models")
    fake_legato_processing = ModuleType("legato.models.processing_legato")
    fake_legato_models.LegatoModel = FakeLegatoModel
    fake_legato_processing.LegatoProcessor = FakeLegatoProcessor
    ...
    monkeypatch.setitem(sys.modules, "legato.models.processing_legato", fake_legato_processing)
    monkeypatch.setattr(adapters_module.AutoConfig, "from_pretrained", staticmethod(lambda _model_id: FakeConfig()))
    monkeypatch.setattr(
        adapters_module.AutoTokenizer,
        "from_pretrained",
        staticmethod(lambda model_id: SimpleNamespace(model_id=model_id)),
    )
    monkeypatch.setattr(
        adapters_module.AutoImageProcessor,
        "from_pretrained",
        staticmethod(lambda model_id, **_kwargs: SimpleNamespace(model_id=model_id)),
    )
...
def test_legato_adapter_rejects_missing_encoder_override(monkeypatch, tmp_path: Path) -> None:
    ...
    fake_legato_processing = ModuleType("legato.models.processing_legato")
    fake_legato_models.LegatoModel = FakeLegatoModel
    fake_legato_processing.LegatoProcessor = object
    fake_legato_package.models = fake_legato_models
...
def test_runner_fails_fast_when_omr_ned_dependency_is_missing(monkeypatch, tmp_path: Path) -> None:
    runner = BenchmarkRunner(
        BenchmarkRunnerConfig(
            output_root=str(tmp_path),
            datasets=("missing",),
    ...
    monkeypatch.setattr("src.benchmark.runner.is_musicdiff_available", lambda: False)

    with pytest.raises(RuntimeError, match="musicdiff"):
        runner.run()


def test_runner_skips_musicdiff_preflight_when_omr_ned_is_not_requested(
...
def test_run_manifest_records_requested_and_effective_our_decode_policy(
    ...
    monkeypatch.setattr("src.benchmark.runner.is_musicdiff_available", lambda: True)
    monkeypatch.setattr(
