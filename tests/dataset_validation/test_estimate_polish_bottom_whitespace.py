...
def test_real_polish_pages_produce_stable_non_null_estimates() -> None:
    ...
    required_images = [sample_dir / name for name in ("0004.png", "0058.png", "0005.png")]
    if any(not path.exists() for path in required_images):
        pytest.skip("Polish validation image analysis fixtures are not available")

    result_a = estimator.analyze_image(required_images[0])
    result_b = estimator.analyze_image(required_images[1])
    result_c = estimator.analyze_image(required_images[2])
