...
from src.pretraining.fcmae.data import (
    FCMAEDataModule,
    FCMAEImageDataset,
    compute_patch_ink_density,
...
def test_fcmae_datamodule_uses_validation_manifest_without_shuffling(tmp_path: Path) -> None:
    image_dir = tmp_path / "images"
    image_dir.mkdir()
    _write_image(image_dir / "train-a.png", (80, 120))
    _write_image(image_dir / "train-b.png", (80, 120))
    _write_image(image_dir / "val-a.png", (80, 120))
    validation_manifest = tmp_path / "validation.txt"
    validation_manifest.write_text("images/val-a.png\n", encoding="utf-8")
    ...
    assert datamodule.val_set is not None
    assert len(datamodule.train_set) == 3
    assert len(datamodule.val_set) == 1
...
def test_reconstruction_logger_can_log_validation_preview(monkeypatch) -> None:
    class FakeExperiment:
        def __init__(self) -> None:
            self.logs = []
    ...
    logger = FakeWandbLogger()
    trainer = SimpleNamespace(logger=logger, global_step=1000)
    pl_module = SimpleNamespace(_latest_val_preview=payload)
    callback = FCMAEReconstructionLogger(
        every_n_steps=1000,
        max_batches=2,
        max_images=1,
        payload_attribute="_latest_val_preview",
        log_key="val/reconstruction_preview_fixed",
    )
    ...
    assert logger.experiment.logs[0][0].keys() == {"val/reconstruction_preview_fixed"}
...
def test_validation_step_uses_deterministic_masks_and_logs_val_metrics() -> None:
    model_config = FCMAEModelConfig(
        patch_size=32,
        mask_ratio=0.6,
        decoder_dim=16,
    ...
    module.log = lambda name, value, **kwargs: logged.append((name, value, kwargs))
    pixel_values = torch.randn(2, 3, 64, 96)
    valid_patch_mask = torch.ones(2, 2, 3, dtype=torch.bool)
    batch = {
        "pixel_values": pixel_values,
        "valid_patch_mask": valid_patch_mask,
    }
    ...
    first_mask = module._latest_val_preview["mask"].clone()
    module.validation_step(batch, 0)
    second_mask = module._latest_val_preview["mask"].clone()

    assert torch.equal(first_mask, second_mask)
    assert "val/loss" in [name for name, _value, _kwargs in logged]
    assert "val/loss_foreground_heavy_masked_patches" in [
        name for name, _value, _kwargs in logged
    ]
