...
from scripts.dataset_generation.dataset_generation_new.renderer import render_sample_with_layout_rescue
...
from scripts.dataset_generation.dataset_generation_new.worker import evaluate_sample_plan
...
def test_failed_5_6_render_can_still_truncate_successfully(monkeypatch):
    ...
    recipe = _make_recipe()
    monkeypatch.setattr(
        "scripts.dataset_generation.dataset_generation_new.worker.build_prefix_candidates",
...
def test_required_over_6_systems_still_go_straight_to_truncation():
    plan = _make_plan()
    recipe = _make_recipe()
    ...
    def fake_render(render_text, recipe, *, seed, renderer):
        if seed == plan.seed + 17:
            return _good_render(system_count=4)
        return _good_render(system_count=7)
