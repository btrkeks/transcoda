...
Examples:
  ...
  ./train.sh validate --checkpoint_path weights/GrandStaff/transcoda-model.ckpt --gpu-type rtx4090
  ./train.sh shell --gpu-type rtx5090 --gpus 1
  ./train.sh queue
  ./train.sh logs
  ./train.sh doctor --checkpoint_path weights/GrandStaff/transcoda-model.ckpt --gpu-type rtx4090
  ./train.sh resources
...
if [ "${COMMAND}" = "logs" ]; then
  ...
  if [ -z "${LOG_JOB_ID}" ]; then
    require_cmd squeue
    LOG_JOB_ID="$(
      squeue -u "${USER}" -h -o "%i %t %j" | awk '$2=="R" && $3 ~ /^transcoda-/ {print $1}' | sort -nr | head -n1
    )"
  fi
  if [ -z "${LOG_JOB_ID}" ]; then
    LOG_JOB_ID="$(
  ...
  if [ -z "${LOG_JOB_ID}" ]; then
    LOG_JOB_ID="$(
      squeue -u "${USER}" -h -o "%i %t %j" | awk '$2=="PD" && $3 ~ /^transcoda-/ {print $1}' | sort -nr | head -n1
    )"
  fi
  if [ -z "${LOG_JOB_ID}" ]; then
    LOG_JOB_ID="$(
...
if [ "${GPUS}" -gt 1 ]; then
  ...
  if ! has_forwarded_override "training.accumulate_grad_batches"; then
    ...
    if [ $((ACCUMULATE_GRAD_BATCHES % GPUS)) -eq 0 ]; then
      echo "[train.sh] auto-scaled training.accumulate_grad_batches to ${SCALED_ACCUMULATE_GRAD_BATCHES} for ${GPUS} GPUs" >&2
    else
      echo "[train.sh] auto-scaled training.accumulate_grad_batches to ${SCALED_ACCUMULATE_GRAD_BATCHES} for ${GPUS} GPUs (rounded down from ${ACCUMULATE_GRAD_BATCHES}/${GPUS})" >&2
    fi
  fi
fi
...
if [ -z "${JOB_NAME}" ]; then
  if [ "${COMMAND}" = "validate" ]; then
    JOB_NAME="transcoda-validate"
  else
    JOB_NAME="transcoda-train"
  fi
fi
