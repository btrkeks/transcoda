...
OURS_CHECKPOINT="weights/GrandStaff/transcoda-model.ckpt"
OURS_STRATEGY=""
OURS_NUM_BEAMS=""
...
Slurm options:
  ...
  --job-name NAME          Slurm job name (default: transcoda-bench)
...
Examples:
  ./scripts/benchmark/submit_slurm.sh \
    --job-name transcoda-bench-synth-small-omrned \
    --datasets synth_small \
    --models ours \
    --metrics omr_ned \
    --ours-checkpoint weights/train_full_page_aug_finetune_v1/transcoda-model.ckpt \
    --output-root outputs/benchmark_train_full_page_aug_finetune_v1_synth_small_omr_ned
...
if [ "${COMMAND}" = "logs" ]; then
  ...
  if [ -z "${LOG_JOB_ID}" ]; then
    LOG_JOB_ID="$(
      squeue -u "${USER}" -h -o "%i %t %j" \
        | awk '$2=="R" && $3 ~ /^transcoda-bench/ {print $1}' \
        | sort -nr \
        | head -n1
    )"
  fi
  if [ -z "${LOG_JOB_ID}" ]; then
    LOG_JOB_ID="$(
      squeue -u "${USER}" -h -o "%i %t %j" \
        | awk '$2=="PD" && $3 ~ /^transcoda-bench/ {print $1}' \
        | sort -nr \
        | head -n1
    )"
  fi
