...
_TENUTO_TRANS = str.maketrans({"~": None})
...
def _remove_tenuto_marks(krn: str) -> str:
    """Remove tenuto marks (~) from note/data fields.
    ...
    for line in lines:
        ...
        for field in fields:
            if field.startswith("*") or field.startswith("="):
                ...
                result_fields.append(field.translate(_TENUTO_TRANS))
...
class CleanupKern:
    ...
    18. Remove tenuto marks (~) from note fields
    19. Remove lines containing **kern or **ekern spine declarations
    ...
    def transform(self, text: str, ctx: NormalizationContext) -> str:
        ...
        text = _remove_tenuto_marks(text)
