...
_FULL_PIPELINE = Pipeline(
    [
        ...
        StripTerminalTerminator(),  # Remove terminal *-
        ...
        # == Spine normalization ==
        MergeSplitNormalizer(),
        CanonicalizeSpineSplits(),  # Coalesce equivalent adjacent split-only interpretation lines
        HoistSpineSplitsToBarline(),  # Move safe mid-measure splits after the previous barline
        DelaySpineMergesToBarline(),  # Move safe mid-measure merges right before the following barline
        # == Repairing ==
