...
9. `HoistSpineSplitsToBarline`
10. `DelaySpineMergesToBarline`
11. `MergeSplitNormalizer`
12. `CanonicalizeSpineSplits`
13. `StripTerminalTerminator`
14. `CapSlurs`
15. `RemoveRedundantTimeSignatures`
16. `CanonicalizeHeaderOrder`
17. `MergeHeaderClefLines` — after reordering brings clef lines together
18. `NormalizeNullKeysig`
19. `CanonicalizeNoteOrder`
20. `OrderNotes(ascending=True)`
21. `SymbolsBeforeSplit`
22. `RemoveContradictoryAccidentals` — must run after `CanonicalizeNoteOrder`
23. `RemoveConflictingBowings`
24. `RemoveNullTies` — canonicalizes repeated ties (`[[`, `]]`, `__`) and removes null ties/slurs; must run after `CanonicalizeNoteOrder`
25. `ValidateSpineOperations` — final semantic check for spine width consistency and valid `*v` merge grouping
