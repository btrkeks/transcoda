...
def _log_dataset_size(datamodule: FCMAEDataModule, logger: WandbLogger | bool) -> int:
    ...
    if isinstance(logger, WandbLogger) and _is_rank_zero():
        payload: dict[str, int] = {"data/num_images_used": image_count}
        val_set = getattr(datamodule, "val_set", None)
        if val_set is not None:
            payload["data/num_validation_images_used"] = len(val_set)
        logger.experiment.config.update(payload, allow_val_change=True)
    return image_count
...
def main() -> None:
    ...
    if config.logging.wandb_enabled and config.logging.log_reconstructions:
        ...
        if config.data.validation_manifest_path is not None:
            callbacks.append(
                FCMAEReconstructionLogger(
                    every_n_steps=config.training.validation_every_n_steps,
                    max_batches=config.logging.log_reconstruction_max_batches,
                    max_images=config.logging.log_reconstruction_max_images,
    ...
    if config.data.validation_manifest_path is not None:
        trainer_kwargs["val_check_interval"] = config.training.validation_every_n_steps
    trainer = L.Trainer(
